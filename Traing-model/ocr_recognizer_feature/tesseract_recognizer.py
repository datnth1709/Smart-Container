...
class tesseract_text_recognize():
    ...
    def recognize_image(self, images):
        ...
        if len(images) > 0:
            results = []
            # for img in images:
            text = pytesseract.image_to_string(images, config=self.config)
            print("text: ", text)
            if len(text) >= 3:
                results.append(text)
...
if __name__ == "__main__":
    # config = ("-l eng --oem 3 --psm 3 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890")
    # img = cv2.imread("ocr_recognizer_feature/images/con5.jpg")
    # text = pytesseract.image_to_string(img, config=config)
    # print("tesseract recognize: ", text)

    recognizer = tesseract_text_recognize()
    text = recognizer.recognize_image("ocr_recognizer_feature/images/con3.jpg")
    print("tesseract recognize preprocessing: ", text)
